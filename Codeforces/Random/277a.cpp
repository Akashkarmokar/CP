...
void run_case(){
    int n,m;
    cin>>n>>m;
    vector<vector<int>> langDoneByPeople(m);
    int notKnw = 0;
    for(int i=0;i<n;i++){
    	int k; cin>>k;
    	if(k==0){
    	...
    	for(int j=0;j<k;j++){
    		int x; cin>>x;
    		x--;
    		langDoneByPeople[x].push_back(i);
    	}
    }
    int component = n;
    DSU dsu(n);
    for(int i=0;i<m;i++){
    	for(int j=0;j<(int)langDoneByPeople[i].size()-1;j++){
    		int a = langDoneByPeople[i][j];
    		int b = langDoneByPeople[i][j+1];
    		component-=dsu.merge(a, b);
    	}
    }
    if(notKnw == n)
    	component++;
